...
function ImageViewer({ collection, currentImage }: ImageViewerProps) {
  ...
  function goForward() {
    if (imageIndex !== collection.length - 1) {
      setImageIndex(imageIndex => imageIndex + 1);
    }
  }

  function getIndexesToPreview(): Photo[] {
    let previewArray: Photo[] = [];
    ...
    collection.map((photo) => {
      ...
      if (collection[imageIndex - 2]) {
        return previewArray.push(photo)
      }

      if (collection[imageIndex - 1]) {
        return previewArray.push(photo)
      }

      if (collection[imageIndex]) {
        return previewArray.push(photo)
      }
  ...
  const photoToShow1 = flickrImageFormatter(collection[imageIndex + 1]) ?? "";
  const photoToShow2 = flickrImageFormatter(collection[imageIndex + 2]) ?? "";
  const photoToShow3 = flickrImageFormatter(collection[imageIndex + 3]) ?? "";
  ...
  return (
    <Style.Container>
      ...
      <article className="image-viewer-preview">
        ...
        <figure>
          <img src={photoToShow1} alt={photoTitle ?? "This is the accessible title"} />
          <figcaption>{photoTitle}</figcaption>
        </figure>
        <figure>
          <img src={photoToShow2} alt={photoTitle ?? "This is the accessible title"} />
          <figcaption>{photoTitle}</figcaption>
        </figure>
        <figure>
          <img src={photoToShow3} alt={photoTitle ?? "This is the accessible title"} />
          <figcaption>{photoTitle}</figcaption>
        </figure>
      </article>
    </Style.Container>
